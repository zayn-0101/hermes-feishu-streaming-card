...
![V3.8.0 飞书卡片主回答与思考工具 timeline 效果](docs/assets/feishu-v38-card-timeline.png)

V3.8.0 起，最终答案保留在主内容区，思考过程和工具调用进入可折叠的辅助 timeline；卡片底部不再重复展示同一份工具调用摘要。
...
- **Docker 同步发布**：`docker-compose.example.yml` 默认示例版本同步到 `v3.8.0`，容器内安装仍使用 `install-docker.sh`。
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `HFC_VERSION` | `latest` | 指定安装版本，例如 `v3.8.0`、`v3.6.6` 或 `main` |
...
| `HFC_VERSION` | `latest`（脚本）/ `v3.8.0`（Compose 示例） | 指定安装 tag 或分支 |
...
从 V3.2.x/V3.3.0/V3.4.x/V3.5.x/V3.6.x/V3.7.x 升级到 V3.8.0 向后兼容，**单 Profile 配置无需任何修改**。如果 Hermes 使用自己的 venv，升级后请重新跑 `setup` 或 `install`，让插件同时进入 Hermes runtime Python 并刷新 hook。V3.8.0 还调整了卡片渲染和 runtime import 诊断，建议升级后执行一次 `doctor --explain`。
