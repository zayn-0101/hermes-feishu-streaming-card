...
- **卡片内交互**：Hermes approval / clarify choices 在公开回调模式渲染成飞书按钮；V3.8.3 起 `/new`、`/reset`、`/model` 等独立 slash 命令也可用独立命令卡片完成确认或选择；localhost/private sidecar 默认保留编号文本 fallback。
...
V3.8.3 补齐 slash 命令交互体验：`/new`、`/reset`、`/undo`、`/model <model>` 这类独立命令的确认不再只能退回灰色文本提示，飞书可展示一张独立命令卡片，用户点击后直接在同一张命令卡片上更新结果。正在运行的 Agent 流式卡片仍只承接 approval、clarify、对话选项和思考/工具 timeline，不和独立命令混在一起。

- **独立确认卡片**：Hermes `_request_slash_confirm()` 会优先走飞书 sidecar `interaction.requested`，按钮选择后调用 Hermes 原 handler。
- **独立模型选择卡片**：当 Hermes 请求 Feishu adapter 的 `send_model_picker(...)` 时，插件会补上 Feishu-only picker，选择模型后回写同一张命令卡片。
- **`/update` 不弹卡片**：`/update` 仍按 Hermes 后台升级命令处理，不做交互按钮卡片，避免把升级流程误当成用户确认。
- **安全 fallback**：sidecar 不可用、卡片应用失败、超时或完成态更新失败时，继续交给 Hermes 原生文本路径，避免命令卡死。

完整发布说明见 [V3.8.3 release notes](docs/release-notes-v3.8.3.md)。
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `HFC_VERSION` | `latest` | 指定安装版本，例如 `v3.8.3`、`v3.6.6` 或 `main` |
...
| `HFC_VERSION` | `latest`（脚本）/ `v3.8.3`（Compose 示例） | 指定安装 tag 或分支 |
...
从 V3.2.x/V3.3.0/V3.4.x/V3.5.x/V3.6.x/V3.7.x/V3.8.0/V3.8.1/V3.8.2 升级到 V3.8.3 向后兼容，**单 Profile 配置无需任何修改**。如果 Hermes 使用自己的 venv，升级后请重新跑 `setup` 或 `install`，让插件同时进入 Hermes runtime Python 并刷新 hook。V3.8.3 保留 V3.8.1 的 Gateway-side delta 合并、V3.8.2 的 timeline 阅读层级，并补上独立 slash 命令卡片；建议升级后执行一次 `doctor --explain`，并在飞书里发送 `/hfc status` 验证 sidecar 状态。
