...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `HFC_VERSION` | `latest` | 指定安装版本，例如 `v3.8.11`、`v3.6.6` 或 `main` |
...
| `HFC_VERSION` | `latest`（脚本）/ `v3.8.11`（Compose 示例） | 指定安装 tag 或分支 |
...
从 V3.2.x/V3.3.0/V3.4.x/V3.5.x/V3.6.x/V3.7.x/V3.8.0-V3.8.10 升级到 V3.8.11 向后兼容，**单 Profile 配置无需任何修改**。如果 Hermes 使用自己的 venv，升级后请重新跑 `setup` 或 `install`，让插件同时进入 Hermes runtime Python 并刷新 hook。V3.8.11 保留 V3.8.10 的群聊诊断和工具详情增强，并修复已接管 `/hfc status` 后仍出现灰色 `Unknown command /hfc` 的真实 Feishu/Lark 竞态；建议升级后执行一次 `doctor --explain`，并在普通会话、话题和目标群聊里各发送一次普通问题、`/hfc status`、`/new` 或 `/model` 验证状态。
